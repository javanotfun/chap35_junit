 ...
 * 처리 메소드에서는 최종적으로 JSP페이지 이름을 리턴하거나 다른 메소드를
 * redirect 형태로 반환 받기도 한다.
 */
...
public class BoardController {
	 ...
	 *	 BoardServiceImple 객채(Bean)이 의존성 주입됨.
...
private BoardService service;
	 ...
	 *    Map 형태로 갖고 있다. url(key- 메소드명(value)
	 */
	
	/* Model model : 모델은 컨트롤러 단에서 저장한 데이터를 M-V-C중에서
	 * view 단에서 찾아 쓸수 있도록 데이터를 보관하는 역할을 한다.
	 * 서블렛 프로젝트에서 request에 저장하던 방식과 유사하지만 request에
	 * 바로 저장되는 것은 아니다. 
	 */
	...
	// int no 파라미터 안에쓰면 integer.parseint 로 변환하는 효과랑 같음 모든 넘어오는 데이터는 문자열임
